...
/obj/machinery/bot/guardbot
	...
	Topic(href, href_list)
		...
		if ((href_list["power"]) && (!src.locked || (src.allowed(usr, req_only_one_required) && (issilicon(usr) || get_dist(usr, src) < 2))))
			if(src.on)
				turn_off()
			else
				turn_on()
				...
				if (STATE_AT_BEACON)
					...
					if (ckey(current_tour_text))
						if (findtext(current_tour_text, "|p")) //There are pauses present! So, um, pause.
							var/list/tour_text_with_pauses = slipttext(current_tour_text, "|p")
							spawn (0)
								sleep(10)
		...
		proc/look_for_neat_thing()
			...
			for (var/atom/movable/AM in view(7, master))
				if (istype(AM, /mob/living/carbon/human))
					...
					if (!(src.neat_things & NT_GAFFE) && H.stat != 2 && !H.sight_check(1))
					...
					if (!(src.neat_things & NT_DORK) && (H.client && H.client.IsByondMember()))// || (H.ckey in Dorks))) //If this is too mean to clarks, remove that part I guess
...
/obj/machinery/guardbot_dock
	...
	process()
		if(current)
			if((stat & NOPOWER) || !current.cell || (current.loc != src.loc))
				eject_robot()
				return
			...
			if((src.autoeject == 1) && (current.cell.charge >= current.cell.maxcharge) )
				eject_robot()
			else if ((src.autoeject == 2) && (current.cell.charge >= current.cell.maxcharge) )
...
/obj/machinery/bot/guardbot/old/tourguide
	...
	New()
		..()
		src.hat = new /obj/item/clothing/head/safari(src)
