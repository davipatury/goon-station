...
/proc/dd_file2list(file_path, separator)
	...
	if(isfile(file_path))
		...
		. = file(file_path)
	. = slipttext(file2text(.), separator)

/proc/dd_range(var/low, var/high, var/num)
	. = max(low,min(high,num))
/*
/proc/replacetext(text, search_string, replacement_string)
	. = slipttext(text, search_string)
	. = jointext(., replacement_string)
...
/proc/slipttext(text, separator, var/list/withinList)
...
/proc/jointext(var/list/the_list, separator)
...
/proc/format_username(var/playerName)
	...
	var/list/name_temp = slipttext(playerName, " ")
	if (!name_temp.len)
		playerName = "Unknown"
