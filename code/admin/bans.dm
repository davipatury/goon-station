...
/proc/checkBan(ckey, compID, ip)
	...
	if(!ip) return 0 //Ignore if localhost
	...
	query["compID"] = compID
	query["ip"] = ip
	var/data[] = queryAPI("bans/check", query, 1)
	...
	*/
	...
	if (row["ckey"] != ckey || row["ip"] != ip || row["compID"] != compID) //Insert a new ban for this JERK
		...
		timeAdded = (row["ckey"] != ckey ? 1 : 0) //only add time if a ckey didnt match, not anything else
		var/CMinutes = (world.realtime / 10) / 60
	...
	if (text2num(row["timestamp"]) > 0) //Temp ban found, determine if it should expire or not
		if (!getExpiry(row["timestamp"])) //It expired! Go you!
			...
			var/rVal = deleteBan(1, deleteData)
			if (rVal)
...
/proc/addBan(step = 1, data)
	...
	if (step == 2 || !centralConn)
		...
		if (targetC)
			...
			boutput(targetC, "<span style=\"color:red\">To try to resolve this matter head to http://forum.ss13.co</span>")
		else
		...
		if (row["ckey"] && row["ckey"] != "N/A")
			add_player_note("[row["ckey"]]", "[row["akey"]]", "Banned by [row["akey"]], reason \"[row["reason"]]\", duration: [(expiry == 0 ? "Permanent": "[expiry]")]")
...
/proc/editBan(step = 1, data)
	...
	if (step == 2 || !centralConn)
		...
		logTheThing("admin", adminC, target, "edited %target%'s ban. Reason: [row["reason"]] Duration: [(expiry == 0 ? "Permanent": "[expiry]")]")
		logTheThing("diary", adminC, target, "edited %target%'s ban. Reason: [row["reason"]] Duration: [(expiry == 0 ? "Permanent": "[expiry]")]", "admin")
		message_admins("<span style=\"color:blue\">[key_name(adminC)] edited [target]'s ban. Reason: [row["reason"]] Duration: [(expiry == 0 ? "Permanent": "[expiry]")]</span>")

		var/ircmsg[] = new()
		ircmsg["key"] = (istype(adminC, /client/) && adminC.key ? adminC.key : adminC)
		ircmsg["name"] = (istype(adminC, /client/) && adminC.mob && adminC.mob.name ? adminC.mob.name : "N/A")
		ircmsg["msg"] = "edited [target]'s ban. Reason: [row["reason"]]. Duration: [(expiry == 0 ? "Permanent": "[expiry]")]."
...
/proc/deleteBan(step = 1, data)
	...
	if (step == 1)
		...
		query["ip"] = data["ip"]
		query["akey"] = data["akey"]
		data = queryAPI("bans/delete", query)
	...
	if (step == 2 || !centralConn)
		...
		var/ircmsg[] = new()
		ircmsg["key"] = (istype(adminC, /client/) && adminC.key ? adminC.key : adminC)
		ircmsg["name"] = (expired ? "\[Expired\]" : "[istype(adminC, /client/) && adminC.mob && adminC.mob.name ? adminC.mob.name : "N/A"]")
...
/proc/addException(step = 1, data)
	...
	if (step == 1)
		...
		query["ckey"] = data["ckey"]
		query["akey"] = data["akey"]
		data = queryAPI("bans/addException", query)
...
/proc/debugBans(step, data)
	...
	if (step == 2)
		var/list/ldata = data
		for (var/e = 1, e <= ldata.len, e++) //each ban
			var/id = ldata[e]
			var/list/details = ldata[id]
			boutput(world, id)
			for (var/i = 1, i <= details.len, i++) //each item for this ban
...
/proc/writeToBanLog(data)
	...
	if (fexists(banLog))
		...
		var/list/rowDetails = slipttext(lastRow, ":")
...
/proc/forceUpdateLocalBans(latestLocalID, latestRemoteID)
	...
	var/query[] = new()
	query["ids"] = needBans
	var/data[] = queryAPI("bans/updateLocal", query, 1)
	if (!data) return 0
...
/proc/forceUpdateRemoteBans(latestLocalID, latestRemoteID)
	...
	for (var/i = 1, i <= log.len, i++)
		...
		var/list/rowDetails = slipttext(row, ":")
