...
var/list/meatland_fx_sounds = list('sound/ambience/meatzone_fx1.ogg','sound/ambience/meatzone_fx2.ogg','sound/ambience/meatzone_fx3.ogg','sound/ambience/meatzone_fx4.ogg')

/area/meat_derelict
	icon_state = "red"
	RL_Lighting = 1

	var/sound/ambientSound = 'sound/ambience/meatzone_amb_slow.ogg'
...
/area/meat_derelict/soviet
	...
	icon_state = "purple"
	ambientSound = 'sound/ambience/meatzone_amb_soviet.ogg'

/area/meat_derelict/boss
	name = "The Heart"
	icon_state = "security"
	ambientSound = 'sound/ambience/meatzone_amb_fast.ogg'
	irradiated = 0.1
...
/obj/critter/monster_door/meat_head //Clash at Meathead
	...
	update_meat_head_dialog(var/message)
		...
		var/list/exploded_sentence = slipttext(message, " ")
...
/obj/item/device/audio_log/meatland_grody
	...
	explode()
		...
		playsound(., "sound/effects/fleshbr1.ogg", 50, 1)
		gibs(.)
		qdel(src)
	...
	Topic(href, href_list)
		...
		if(href_list["key"])
			if(istype(usr.equipped(), /obj/item/device/key/cheget) && !src.inserted_key)
				...
				playsound(host.loc, 'sound/effects/thunk.ogg', 30, 1)
				spawn(10)
					if(src.inserted_key)
			...
			if(src.inserted_key)
				...
				playsound(src.loc, "sound/effects/thunk.ogg", 30, 1)
...
/obj/item/gun/gibgun
	...
	shoot(var/target,var/start,var/mob/user,var/POX,var/POY)
		...
		if((last_shot + 15) <= world.time)
			...
			playsound(T, "sound/effects/fleshbr1.ogg", 40, 1)
