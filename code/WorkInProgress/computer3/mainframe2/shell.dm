				...
				switch(lowertext(command))
					...
					if ("talk")
						if (pipetemp)
							...
							if(istype(command_list))
								command_list += slipttext(pipetemp, " ")
					...
					if ("logout","logoff")
						...
						if (pipetemp)
							...
							var/list/templist = slipttext(command, "/")
							if (!templist.len)
							...
							rec.fields = slipttext(pipetemp, "|n")
							rec.name = recname
				...
				switch ( lowertext(current_command) )
					...
					if ("/") //(1X 2X -- (1X / 2X))
						...
						if (script_isNumResult(stack[stack.len], stack[stack.len-1]))
							...
							var/list/explodedString = slipttext("[stack[stack.len-1]]", "[stack[stack.len]]")
