...
*/
	...
	proc/trim_path(var/filepath)
		var/list/filelist = slipttext(filepath, "/")
...
/datum/computer/file/mainframe_program/utility/cp
	...
	initialize(var/initparams)
		...
		var/list/copypath = slipttext(initlist[2], "/")
		var/copyname = null
		if (adjust_name)
...
/datum/computer/file/mainframe_program/utility/ln
	...
	initialize(var/initparams)
		...
		var/prefixroot = dd_hasprefix(initlist[2], "/")
		var/list/linkpath = slipttext(initlist[2], "/")
		var/linkname = null
		if (linkpath.len)
...
/datum/computer/file/mainframe_program/utility/mkdir
	...
	initialize(var/initparams)
		...
		while (initlist.len)
			...
			var/list/dirpath = slipttext(initlist[1], "/")
			var/dirname = null
			if (dirpath.len)
...
/datum/computer/file/mainframe_program/utility/chown
	...
	initialize(var/initparams)
		...
		var/list/newlist = slipttext(initlist[1], ":") //New owner/group values should given in form owner:group
			...
			if (opts["l"] || opts["x"])
				...
				for (var/path in unaff)
					...
					if (!istype(C2))
						...
						return
					archive.add_file(C2)
				var/list/arcparts = slipttext(arcfile, "/")
